...
public:
    ...
    MeshData() : vertex_count_(0), primitive_type_(GL_TRIANGLES) {

    }

    explicit MeshData(size_t vertex_count) : vertex_count_(0), primitive_type_(GL_TRIANGLES) {
        vertices_.reserve(vertex_count);
    }

    explicit MeshData(std::vector<VertexType>&& vertices) : vertex_count_(0), primitive_type_(GL_TRIANGLES) {
    ...
    std::unique_ptr<BoundingShape>&& moveBoundingShape() {
    ...
    void computeShape() {
        ...
        setBoundingShape(std::make_unique<BoundingCuboid>(min, max - min));
    }
...
public:
    ...
    Mesh() : Mesh(getShaderUtil<VertexShaderProgram>()) {
        
    }

    explicit Mesh(VertexShaderProgram* program) :
        ShaderNode(program),
        vertex_array_id_(0),
        vertex_buffer_id_(0),
        num_vertices_(0),
    ...
    // Loads a mesh described by an Mesh object
    // Cleanup specifies whether it should unbind the VAO after it has finished creating it
    void loadMeshData(MeshDataType* data, bool is_static, bool cleanup = true) {
        FTAssert(!is_loaded_, "Trying to load mesh data for already loaded mesh");
    ...
    void setMeshData(MeshDataType* data) {
