...
Font::Font(const std::string& path, const char* cache) : font_texture_(nullptr) {

    auto extension = path.substr(path.find_last_of(".") + 1);

    if (extension == "ttf") {
        font_texture_ = std::make_shared<FontTexture>(texture_atlas_new(1024, 1024, 1));
        ...
        ttvfs::File *fr = Engine::getFileManager()->getFile(path);
        FTAssert(fr && fr->open("rb"), "Failed to find Font %s", path.c_str());

        font_ = FontUtils::deserializeFont(fr);
        fr->close();

        font_texture_ = std::make_shared<FontTexture>(font_->atlas);
    } else {
...
std::unique_ptr<FontMeshData> Font::generateMeshForString(const std::basic_string<wchar_t>& text, float point_size) {
    size_t length = text.length();
    auto data = new FontMeshData(4 * length, 6 * length);
    populateMeshDataForString(data, text, point_size);
    return std::unique_ptr<FontMeshData>(data);
}
...
void Font::add_glyph(const char *codepoint) {
    size_t i, j;
    FT_Library library;
    FT_Face face;
...
void Font::populateMeshDataForString(FontMeshData* data, const std::basic_string<wchar_t>& text, float point_size) {
    ...
    for (i = 0; i < length; ++i) {
        ...
        if (glyph != nullptr) {
            ...
            // the FontMeshData::getGlyphBounds
    ...
    glyph_starts.push_back(pen.x);
    data->setBoundingShape(std::make_unique<BoundingCuboid>(glm::vec3(pen.x, maxHeight, 0)));
}
