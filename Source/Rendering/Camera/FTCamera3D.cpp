...
FTCamera3D::FTCamera3D() : position_(0, 0, 0), rotation_euler_radians(0, 0), rotation_dirty_(true), update_view_frustrum_(true), fov_(45) {
	//FTDirector::getSharedInstance()->getWindowSizeChangeEventHandler()->Connect(this, &FTCamera::screensizeChange);
	//preDraw();
	//regenerateViewFrustrum();
};

FTCamera3D::~FTCamera3D() {
	//FTDirector::getSharedInstance()->getWindowSizeChangeEventHandler()->Disconnect(this, &FTCamera::screensizeChange);
}
