...
void FTCamera3D::regenerateViewFrustrum() {
    ...
    frustrum_planes_sign_flipped_[0] = glm::vec3(-data0.x, -data0.y, -data0.z);
    // Far Plane
    glm::vec4& data1 = frustrum_planes_[1];
    data1 = normalizeVec4(row4 - row3);
    frustrum_planes_sign_flipped_[1] = glm::vec3(-data1.x, -data1.y, -data1.z);

    // Left Plane
    glm::vec4& data2 = frustrum_planes_[2];
    data2 = normalizeVec4(row4 + row1);
    frustrum_planes_sign_flipped_[2] = glm::vec3(-data2.x, -data2.y, -data2.z);

    // Right Plane
    glm::vec4& data3 = frustrum_planes_[3];
    data3 = normalizeVec4(row4 - row1);
    frustrum_planes_sign_flipped_[3] = glm::vec3(-data3.x, -data3.y, -data3.z);

    // Top Plane
    glm::vec4& data4 = frustrum_planes_[4];
    data4 = normalizeVec4(row4 + row2);
    frustrum_planes_sign_flipped_[4] = glm::vec3(-data4.x, -data4.y, -data4.z);

    // Bottom Plane
    glm::vec4& data5 = frustrum_planes_[5];
    data5 = normalizeVec4(row4 - row2);
    frustrum_planes_sign_flipped_[5] = glm::vec3(-data5.x, -data5.y, -data5.z);
}
