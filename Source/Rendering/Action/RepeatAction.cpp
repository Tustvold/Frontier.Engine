...
RepeatAction::RepeatAction(std::unique_ptr<Action>&& action, int repeat_count) : repeat_count_(repeat_count), remaining_count_(repeat_count_), action_(std::move(action)) {
...
void RepeatAction::onStart(Node* node) {
    Action::onStart(node);
...
void RepeatAction::onUpdate(Node* node, const UpdateEvent& event) {
