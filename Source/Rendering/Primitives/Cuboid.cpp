...
Cuboid::Cuboid(const glm::vec3& origin, const glm::vec3& size, const glm::vec3 face_colors[6], VertexColorShaderProgram* shader) :
    FTCuboidBase_(shader) {
    load(origin, size, face_colors);
}

Cuboid::Cuboid(const glm::vec3& origin, const glm::vec3& size, const glm::vec3& color, VertexColorShaderProgram* shader) : FTCuboidBase_(shader) {
    glm::vec3 colors[] = { color, color, color, color, color, color };
    load(origin, size, colors);
}

void Cuboid::load(const glm::vec3& origin, const glm::vec3& size, const glm::vec3 face_colors[6]) {
    auto data = std::make_shared<IndexedMeshData<VertexColorNormal<glm::vec3>, uint16_t>>(4 * 6, 36);

    auto vertices = data->getVertices().data();
    auto indices = data->getIndices().data();

    VertexColorNormal<glm::vec3> vertex;
    int cur_index = 0;
    int vertex_index = 0;
    int indices_index = 0;
    ...
    setBoundingShape(std::make_unique<BoundingCuboid>(origin, origin + size));
}
